...
description: Update weekly ministry signup counts (CONNECT, SERVE, BUILD, FAVOR DNA) in the CIW / SIGNUPS tab, sourced entirely from Rock RMS workflow types. Fluro has been decommissioned — Rock is the sole source. Trigger on "update Sunday signups", "CIW signups", or columns E–H of the CIW sheet.
---
...
- `references/source-map.md` — column→source mapping, Rock workflow type IDs, write mechanics.
...
**All four signups live in Rock RMS. Fluro has been decommissioned** — do not query it, and do not attempt any Fluro/Rock dedup or union. BUILD/FAVOR DNA migrated 2026-06-14; CONNECT/SERVE migrated in the July 2026 cutover (Rock CONNECT type 26 live from 2026-06-03; Rock SERVE type 49 first submissions 2026-07-12). As of the 2026-08-03 run, residual Fluro counts for CONNECT/SERVE were confirmed at 0 — Rock is the sole source for all four columns; treat any earlier reference to "Fluro union" or "cutover aggregation" in older notes as historical only.
...
2. **Compute counts for the window** (recompute live; counts drift through the day). All four counts come from Rock only — no Fluro querying, no dedup/union step. **Easiest path: `scripts/signups-week-count.py`** computes all four Rock counts for a given window in one shot. Manual mechanics:
   - **Rock (all four)** — count workflow rows per type in the window. Two ways:
     ...
     - *REST API* (needed for CONNECT — see junk-filter below — and any attribute-level detail): prod Rock API key at `~/Git/connect.favor.church/.env.production` (`ROCK_API_KEY`; header `Authorization-Token`) — this repo was formerly named `favor-connect-portal`; if the key file has moved again, search `~/Git` for `.env.production` containing `ROCK_API_KEY`. `GET https://rock.favor.church/api/Workflows?$filter=...&$select=Id,Name,Status,CreatedDateTime&$top=500`. Rock `CreatedDateTime` is already Asia/Manila.
...
4. **Verify.** Read back `A{row}:H{row}` (FORMATTED_VALUE); confirm `A` (date) and `E–H` (counts) landed and that `B/C/D` are untouched. Spot-check one Rock count (`rock_entity search`).
...
All four created 2026-06/07 and don't normally need re-creating. (DataViews filter by workflow type **all-time**, not by week — a "click to see the signups" reference; the weekly number lives in the cell.) To recreate/maintain a DataView, use the prod **Rock API key** at `~/Git/connect.favor.church/.env.production` (`ROCK_API_KEY`; header `Authorization-Token: <key>` against `https://rock.favor.church/api`). Do not read that repo — only the env file. The read-only MCP cannot write DataViews.
...
- `scripts/fluro-week-count.mjs` is retired (Fluro decommissioned) — kept only for historical reference, not part of the live workflow.
