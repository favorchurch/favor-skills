...
description: >
  Use when refreshing, recounting, or incrementally updating the unique-church count for
  Favor Conference 2026 (OPEN ACCESS) in the "Church Count (Normalized)" tab of the Google
  Sheet — folding in new MASTERLIST signups, recomputing counts, merging duplicate rows,
  pruning dead rows, or auditing the naming convention. Triggers: "update the church count",
  "recount unique churches", "add new churches from the masterlist", "how many unique churches
  now", "/update-unique-churches". Writes via the Composio CLI (GOOGLESHEETS_* tools).
---
